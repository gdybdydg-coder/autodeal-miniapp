...
function renderSavedView() {
  ...
  $("saveHint").textContent=editingSearch?.scope==="local"?"Зміни збережуться лише на цьому пристрої.":
    ...
    "Збережемо в акаунті Telegram на паузі. Далі ввімкни тумблер біля підписки.";
...
function subscriptionCard(item,options) {
  ...
  if(options.cloud&&options.toggle) {
    const activate=managerButton("",async()=>{
        if(activate.disabled||window.AutoDealCloudSearches?.isBusy?.()) return;
        if(!options.canToggle) return openSettings();
        activate.disabled=true;activate.setAttribute("aria-busy","true");
        try {await options.toggle();}
        finally {activate.disabled=false;activate.setAttribute("aria-busy","false");}
      });
    activate.className="subscription-toggle";
    activate.setAttribute("role","switch");
    activate.setAttribute("aria-checked",String(!!item.enabled));
    activate.setAttribute("aria-label","Сповіщення для підписки «"+item.name+"»");
    activate.title=item.enabled?"Вимкнути сповіщення":options.canToggle?"Увімкнути сповіщення":"Підключити чат бота";
    const track=document.createElement("span");track.className="subscription-toggle-track";
    track.setAttribute("aria-hidden","true");activate.append(track);
    aside.append(activate);
  }
